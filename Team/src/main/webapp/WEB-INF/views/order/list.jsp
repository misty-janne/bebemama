...
<script type="text/javascript">
	function managerdelete(pno) {
		var isOk = confirm("삭제하시겠습니까?");
		if (isOk == true) {
			location.href = "/order/delete/" + pno;
		}
	}
</script>
<style>
...
<jsp:include page="../includes/header.jsp" />
	<div class="container">
		<div class="row text-center">
			<h1 style="color:hotpink;">**전체 주문 목록**</h1>
		</div>
		...
		<div class="row">
			<table class="table">
				<thead>
					<tr>
						<th class="th-center">pno</th>
						<th class="th-center">id</th>
						<th class="th-center">주문서</th>
						<th class="th-center">주문일자</th>
						<th class="th-center">합계금액</th>
				...
				<tbody>
					...
					<c:forEach items="${list }" var="dto">
						<tr>
							<td class="td-center">${dto.pno }</td>
							<td class="td-center">${dto.id }</td>
							<td>
								<a href="#">
								상세보기
								</a>
		...
		<nav aria-label="Page navigation">
  		<ul class="pagination">
    		<li>
      		<a href="/order/list?curPage=${to.curPage > 1 ? to.curPage-1 : 1 }" aria-label="Previous">
        		<span aria-hidden="true">&laquo;</span>
      		</a>
    		</li>
    		<c:forEach begin="${to.beginPageNum }" end="${to.stopPageNum }" var="page">
    			<li class="${to.curPage == page? 'active' : '' }">
    			<a href="/order/list?curPage=${page}">${page }</a>
    			</li>
    		</c:forEach>
			<li>
      		<a href="/order/list?curPage=${to.curPage < to.totalPage ? to.curPage+1 : to.curPage }" aria-label="Next">
